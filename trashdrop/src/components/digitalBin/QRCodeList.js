...
const QRCodeCard = ({ pickup, onCancel, onShare, onRefresh, isNewlyCreated = false, onExpanded = null }) => {
  ...
  // Helper to check if URL is a web URL (not a data URL)
  const isWebUrl = (url) => url && url.startsWith('http') && !url.startsWith('data:');
  
  // Reset autoGenerated flag when the bin changes
  useEffect(() => {
    autoGeneratedRef.current = false;
  }, [pickup.id]);
  
  // Load QR code from local storage when component mounts - with auto-generation for new bins
  useEffect(() => {
    const loadQRCode = async () => {
      if (pickup.id) {
        try {
          const qrData = await getQRCode(pickup.id);
          
          // Check if we got valid QR data with a data URL (starts with 'data:')
          const hasValidQrCode = qrData && qrData.qrCodeUrl && qrData.qrCodeUrl.startsWith('data:');
          
          if (hasValidQrCode) {
            setQrCodeData(qrData);
            setQrCodeUrl(qrData.qrCodeUrl);
            console.log('Loaded QR code for bin:', pickup.id);
          } else {
            console.log('No valid QR code data URL found for bin:', pickup.id);
            
            // Auto-generate QR code if:
            // 1. No valid data URL exists (first time or expired)
            // 2. Haven't already auto-generated for this location
            if (!autoGeneratedRef.current) {
              console.log('Auto-generating QR code for bin:', pickup.id, 'isNewlyCreated:', isNewlyCreated);
              autoGeneratedRef.current = true;
              try {
                const newQrData = await storeQRCode(pickup.id, null, {
                  locationId: pickup.location_id,
                  syncToSupabase: true
                });
                
                if (newQrData && newQrData.qrCodeUrl) {
                  setQrCodeData(newQrData);
                  setIsImageLoaded(false); // Reset image loaded state for new QR code
                  setQrCodeUrl(newQrData.qrCodeUrl);
                  console.log('Auto-generated QR code for bin:', pickup.id);
                }
              } catch (genError) {
                console.error('Failed to auto-generate QR code:', genError);
                autoGeneratedRef.current = false; // Reset on error so it can retry
              }
            }
          }
        } catch (error) {
          console.error('Error loading QR code:', error);
          autoGeneratedRef.current = false; // Reset on error
        }
      }
    };
    
    loadQRCode();
  }, [pickup.id, pickup.qr_code_url, isNewlyCreated]);
  ...
  // Notify parent when newly created bin is expanded (one-time effect)
  useEffect(() => {
    if (isNewlyCreated && isExpanded && onExpanded) {
      onExpanded();
    }
  }, [isNewlyCreated, isExpanded, onExpanded]);
  ...
  const handleRefreshQR = async (e) => {
    ...
    setIsLoading(true);
    try {
      console.log('Refreshing QR code for bin:', pickup.id);
      ...
      if (newQrData) {
        setQrCodeData(newQrData);
        setQrCodeUrl(newQrData.qrCodeUrl);
        console.log('Generated new QR code for bin:', pickup.id);
          ...
          <div className="flex-1">
            ...
            {pickup.fee && (
              <p className="mt-1 text-sm font-semibold text-green-700 dark:text-green-400">
                GHS {parseFloat(pickup.fee).toFixed(2)}
              </p>
            )}
          </div>
          <button 
            ...
            <div className="space-y-4">
              ...
              {pickup.fee && (
                <div className="flex items-start">
                  <FaInfoCircle className="mt-1 mr-2 text-gray-400 dark:text-gray-500 flex-shrink-0" />
                  <div>
                    <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">Service Fee</h4>
...
const QRCodeList = ({ 
  ...
  newlyCreatedBinId = null,
  onNewBinExpanded = null
}) => {
  if (isLoading) {
    return (
    ...
    <div className="space-y-4">
      {pickups.map((pickup) => (
        <QRCodeCard 
          ...
          isNewlyCreated={pickup.id === newlyCreatedBinId}
          onExpanded={onNewBinExpanded}
        />
      ))}
    </div>
  );
