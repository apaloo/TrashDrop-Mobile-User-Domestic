...
const QRCodeCard = ({ pickup, onCancel, onShare, onRefresh, isNewlyCreated = false, onExpanded = null }) => {
  ...
  // Reset autoGenerated flag when location changes
  useEffect(() => {
  ...
  useEffect(() => {
      ...
      if (pickup.location_id) {
        try {
          ...
          if (hasValidQrCode) {
            ...
            console.log('No valid QR code data URL found for location:', pickup.location_id);
            ...
            if (!autoGeneratedRef.current) {
              console.log('Auto-generating QR code for:', pickup.location_id, 'isNewlyCreated:', isNewlyCreated);
              autoGeneratedRef.current = true;
              try {
                const newQrData = await storeQRCode(pickup.location_id, null, {
                ...
                if (newQrData && newQrData.qrCodeUrl) {
                  ...
                  console.log('Auto-generated QR code for location:', pickup.location_id);
                }
              } catch (genError) {
    ...
    loadQRCode();
  }, [pickup.location_id, pickup.qr_code_url, pickup.id, isNewlyCreated]);
