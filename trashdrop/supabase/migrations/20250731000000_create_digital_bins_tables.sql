-- Create the bin_locations table if it doesn't exist
...
-- Create the digital_bins table if it doesn't exist
...
-- Allow users to see their own digital bins
CREATE POLICY "Users can view their own digital bins"
  ON public.digital_bins
  FOR SELECT
  USING (auth.uid() = user_id);

-- Allow users to create digital bins
CREATE POLICY "Users can create digital bins"
  ON public.digital_bins
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Allow users to update their own digital bins
...
-- Create a trigger to automatically update the updated_at column for bin_locations
...
-- Create a trigger to automatically update the updated_at column for digital_bins
