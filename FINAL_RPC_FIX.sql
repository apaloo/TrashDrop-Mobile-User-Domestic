-- FINAL FIX: Target the specific column 5 type mismatch issue
-- The error shows "text does not match expected type bigint in column 5"

-- Drop and recreate with proper column 5 (total_bags) handling
...
BEGIN
  ...
  WITH dashboard_stats AS (
    SELECT 
      ...
      -- FIXED: Ensure total_bags is properly cast to BIGINT
      CASE 
        WHEN us.total_bags IS NOT NULL AND us.total_bags ~ '^[0-9]+$' THEN us.total_bags::BIGINT
        WHEN us.total_bags IS NOT NULL THEN 0::BIGINT
        ELSE 0::BIGINT
      END as total_bags,
