-- Missing RPC Functions for Onboarding Service (Safe Version)
-- This version drops existing functions before recreating them to avoid conflicts

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS get_user_has_bags_selection(UUID);
DROP FUNCTION IF EXISTS dismiss_onboarding(UUID);
DROP FUNCTION IF EXISTS process_qr_scan(UUID, TEXT);
DROP FUNCTION IF EXISTS create_digital_bin(UUID, UUID);
DROP FUNCTION IF EXISTS create_digital_bin(UUID);
...
-- 3. PROCESS QR SCAN (Onboarding Version)
...
-- 4. CREATE DIGITAL BIN (Onboarding Version)
