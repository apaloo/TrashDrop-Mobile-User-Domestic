-- COMPLETE FIX: Drop and recreate RPC function with proper BIGINT casting
-- This will completely replace the function to fix type mismatches

-- Step 1: Completely drop the existing function
DROP FUNCTION IF EXISTS get_dashboard_data_optimized(UUID, INT);

-- Step 2: Create the function with all proper BIGINT casts
...
BEGIN
  ...
  WITH dashboard_stats AS (
    ...
    LEFT JOIN (
      SELECT 
        user_stats.user_id,
        user_stats.available_bags::BIGINT,
        user_stats.total_bags::BIGINT,
...
-- SELECT * FROM get_dashboard_data_optimized('3e5218d6-7742-4263-bed0-8b35894c2794', 1);
